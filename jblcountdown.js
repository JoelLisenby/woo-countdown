...
var jblcountdown = function( id, begin, end ) {
	...
	jt.render = function() {
		...
		if( jt.timeleft > 0 && jt.now.getTime() > jt.begin.getTime() ) {
			html += jt.days > 0 ? '<div class="days"><span class="time">'+ jt.days +'</span><span class="label">Day'+ ( jt.s( jt.days ) ? 's' : '' ) +'</span></div>' : '';
			html += jt.hours > 0 ? '<div class="hours"><span class="time">'+ jt.hours +'</span><span class="label">Hour'+ ( jt.s( jt.hours ) ? 's' : '' ) +'</span></div>' : '';
			html += jt.minutes > 0 ? '<div class="minutes"><span class="time">'+ jt.minutes +'</span><span class="label">Minute'+ ( jt.s( jt.minutes ) ? 's' : '' ) +'</span></div>' : '';
			html += jt.seconds >= 0 ? '<div class="seconds"><span class="time">'+ jt.seconds +'</span><span class="label">Second'+ ( jt.s( jt.seconds ) ? 's' : '' ) +'</span></div>' : '';
